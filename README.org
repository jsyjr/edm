...
Edm is new and experimental. While it works very well on the subset
...
packages, install =edm= with =M-x package-install=.
...
Edm divides the frame into 2 areas, the master area and the stack.
The master area contains the largest window(s), and other windows are
placed in the stack. Edm tries to place the master area to the
...
Edm arranges windows according to just a few parameters: the size
...
=edm-keymap-prefix= to change the prefix.
...
See also =(edm-setup-dwm-keys)= to set up alternative dwm-like key
bindings.

* Scope

Edm does not provide workspaces or window rules, instead delegating
